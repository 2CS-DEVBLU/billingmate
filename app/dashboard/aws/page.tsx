...
import { AwsBillingChart } from "@/components/aws-billing-chart"
import { AwsResourceBreakdown } from "@/components/aws-resource-breakdown"
import { AwsRecommendations } from "@/components/aws-recommendations"
import { AwsProductsList } from "@/components/aws-products-list"
...
export default async function AwsDashboardPage({
  ...
  const previousMonthCost = billingHistory?.[1]?.total_cost || 0
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-indigo-950 to-slate-900">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-white mb-2">AWS Cost Analytics</h1>
          ...
          <div className="flex gap-3 items-center">
            {isAdmin && (
              <Link href="/dashboard/integrations/aws">
                <Button
                  variant="outline"
                  className="group relative overflow-hidden border-2 border-orange-500/50 bg-orange-950/50 text-orange-200 hover:text-white hover:border-orange-400 transition-all duration-300 shadow-lg hover:shadow-orange-500/25"
                >
                  <div className="absolute inset-0 bg-gradient-to-r from-orange-600/20 to-amber-600/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
            ...
            <SyncDataButton integrationId={integration.id} canSync={canSync} cooldownRemaining={cooldownRemaining} provider="aws" />
          </div>
        </div>
        ...
        <div className="mb-8">
          <AwsBillingChart billingHistory={billingHistory || []} />
        </div>

        {displayProducts.length > 0 ? (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
            <AwsProductsList resourceCosts={displayProducts} timeRange={timeRange} />
          ...
          <Card className="bg-slate-800/50 border-slate-700 mb-8">
            <CardHeader>
              ...
              <CardDescription className="text-slate-400">
                No detailed service data available yet. Sync your account to fetch cost information.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <p className="text-sm text-slate-400">
                Service-level cost breakdown will appear here after your first successful sync with AWS Cost Explorer.
              </p>
            </CardContent>
          </Card>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <AwsResourceBreakdown resourceCosts={displayProducts} />
          <AwsRecommendations

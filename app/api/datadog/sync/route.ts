...
import { DatadogAPI } from "@/lib/datadog/api"
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log("[v0] Starting Datadog sync for integration:", integrationId, "Log ID:", logId)
    ...
    const appKey = integration.config?.app_key

    if (!apiKey || !appKey) {
      throw new Error("Missing Datadog API credentials")
    }

    const ddAPI = new DatadogAPI({ apiKey, appKey })
    ...
    for (let i = 0; i < 12; i++) {
      ...
      console.log("[v0] Fetching Datadog data for", year, month)

      try {
        const billingData = await ddAPI.fetchBillingData(year, month)
        ...
        if (billingData.resources.length > 0) {
          ...
          if (rcError) {
            ...
            console.log("[v0] Inserted", resourceCosts.length, "Datadog resource costs")
          }
        }

        // Generate and store recommendations (only for current month)
        if (i === 0) {
          const recommendations = await ddAPI.generateRecommendations(billingData)
    ...
    console.log("[v0] Datadog sync completed:", syncResults)

    return NextResponse.json({
      success: true,
      message: "Datadog billing data synced successfully",
      results: syncResults,
      logId,
    })
  } catch (error) {
    console.error("[v0] Datadog sync error:", error)
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to sync Datadog billing data" },
      { status: 500 },
    )
  }
}
